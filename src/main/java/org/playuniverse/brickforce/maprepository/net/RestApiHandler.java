...
public class RestApiHandler implements RequestHandler {
    ...
    public boolean handleRequest(HttpSender sender, HttpWriter writer, ReceivedRequest request) throws Exception {
        ...
        if (id == -8) {
            new JsonAnswer(StandardContentType.JSON).code(ResponseCode.BAD_REQUEST).respond("error", "An id may only be positive, 0 or -1")
                .write(writer);
            return true;
        }
    ...
    private long validateApiPath(String[] path) {
        ...
        if (path[1].equalsIgnoreCase("list")) {
            return -2;
        }
    ...
    private void handleGet(HttpSender sender, HttpWriter writer, ReceivedRequest request, long id) throws Exception {
        if (id == -1) {
            new JsonAnswer(StandardContentType.JSON).code(ResponseCode.BAD_REQUEST)
                .respond("error", "A map can only have an id higher or equal to zero").write(writer);
            return;
        }
        if(id == -2) {
    ...
    private void handleUpload(HttpSender sender, HttpWriter writer, ReceivedRequest request, long id) throws Exception {
        ...
        if (!request.hasHeader("Mode-Mask")) {
            new JsonAnswer(StandardContentType.JSON).code(ResponseCode.BAD_REQUEST).respond("error", "Mode-Mask header is missing!")
                .write(writer);
            return;
        }
        int modeMask = getModeMask(request.getHeader("Mode-Mask"));
        if (modeMask == -1) {
            new JsonAnswer(StandardContentType.JSON).code(ResponseCode.NOT_ACCEPTABLE).respond("error", "Invalid Mode-Mask!").write(writer);
            return;
        }
        ...
        StoreState state = storage.store(map.copyWithId(id).setModeMask(modeMask));
        if (state == StoreState.FAILED) {
            new JsonAnswer(StandardContentType.JSON).code(ResponseCode.INTERNAL_SERVER_ERROR)
                .respond("error", "Unable to save map to storage").write(writer);
            return;
        }
        new JsonAnswer(StandardContentType.JSON).code(ResponseCode.OK).respond("state", state.name()).respond("id", new JsonLong(id))
            .write(writer);
    }

    private int getModeMask(Object header) {
        if (header instanceof Number) {
            return Math.max(-1, ((Number) header).intValue());
        }
        if (header instanceof String) {
            try {
                return Math.max(-1, Integer.parseInt((String) header));
            } catch (NumberFormatException exp) {
                return -1;
            }
        }
